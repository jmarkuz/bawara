...
object OrderedRedeliverer {
  /**
   * Props for creating a [[OrderedRedeliverer]].
   */
  def props(retryTimeout: FiniteDuration) = Props(classOf[OrderedRedeliverer], retryTimeout)
  ...
  /**
   * Private message used only inside of the [[OrderedRedeliverer]] to signalize a tick of its retry timer.
   */
 ...
 * </pre>
 *
 * ... will result in the second requester getting a [[OrderedRedeliverer.Busy]] message with [[UUID]]s
 * of both his request and currently-processed one.
 *
 * @param retryTimeout how long to wait for the [[OrderedRedeliverer.Ack]] message
 */
class OrderedRedeliverer(retryTimeout: FiniteDuration) extends Actor with FSM[OrderedRedeliverer.State, OrderedRedeliverer.Data] {
  import OrderedRedeliverer._
  ...
  onTransition {
    case Idle -> AwaitingAck => println("Got things to do!")
  }
...
class Receiver extends Actor {
  ...
  def receive = {
    case OrderedRedeliverer.Ackable(from, msg, uuid) =>
      val goingToSendAck = shouldSendAck
      println(s"""  [Receiver] got "$msg"; ${if (goingToSendAck) "" else " ***NOT***"} going to send Ack this time""")
      // Send a [[OrderedRedeliverer.Ack]] -- if they're lucky!
      if (goingToSendAck) sender() ! OrderedRedeliverer.Ack(uuid)
  }
}
...
class Requester extends Actor {
  ...
  /*
   * Create a [[OrderedRedeliverer]] and a [[Receiver]].
   */
  val redeliverer = context.actorOf(OrderedRedeliverer.props(retryTimeout = 3.seconds))
  val receiver = context.actorOf(Props(classOf[Receiver]))
  ...
  def receive = {
    case Tick =>
       ...
       */
      redeliverer ! OrderedRedeliverer.Deliver(receiver, msg, uuid)
...
object FsmRedelivery extends App {
   ...
   */
  system.actorOf(Props(classOf[Requester]))
